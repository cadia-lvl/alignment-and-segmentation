...
cat "$pron" <(echo "<unk> oov") <(echo "<laughter> LAUGHTER") \
<(echo "<coughing> COUGHING") <(echo "<noise> NOISE") \
<(echo "<danish> oov") <(echo "<english> oov") \
<(echo "<foreign> oov") <(echo "<norwegian> oov") \
<(echo "<swedish> oov") <(echo "<spanish> oov") \
<(echo "<german> oov") \
...
for w in sil oov LAUGHTER COUGHING NOISE; do echo $w; done > "$dictdir"/silence_phones.txt
