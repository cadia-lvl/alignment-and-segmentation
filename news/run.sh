...
#           2021 Judy Fong (Reykjavik University) - lvl@judyyfong.xyz
...
# TODO: change description so it doesn't rely on diarization data
...
# Extract the same data as for diarization(ruvdi) to be able to assign
# speaker IDs later.
...
datadir="$data"/one-news-data
expdir="$exp"/one-news-data/segmentation
mfcc="$mfcc"
...
if [ $stage -le 1 ]; then
  ...
  mkdir -p "$corpusdir"/888
  for path in "$corpusdir"/wav/*; do
    file=$(basename "$path")
    # TODO: From a list of recording IDs get the text in the right folder and filename
    # php local/extract_vtt.php "${file%.*}" "$datadir"/888
  done
fi
...
if [ $stage -le 2 ]; then
  ...
  for file in "$corpusdir"/888/*.txt; do
    name=$(basename "$file")
    python3 local/extract_text.py \
    "$file" "$datadir"/transcripts
  done
...
if [ $stage -le 3 ]; then
  echo 'Change individual text files into one'
  cat "$datadir"/transcripts/* >> "$datadir"/raw_text
fi
...
if [ $stage -eq 8 ]; then
  # TODO: change script so it doesn't rely on diarization data
  # WHAT TO DO NOW?!
